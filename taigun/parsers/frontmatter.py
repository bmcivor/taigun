...
class FrontmatterParser:
    """Parses YAML frontmatter from a markdown ticket file.

    Validates required fields, rejects unknown keys, and constructs a
    partially populated ticket dataclass with body fields left empty.
    """
    ...
    def parse(self, text: str) -> tuple[dict, str]:
        ...
        if ticket_type not in self.TICKET_TYPES:
            raise ParseError(
                ...
                f" Must be one of: {', '.join(sorted(self.TICKET_TYPES))}"
            )

        if "tags" in metadata:
            metadata["tags"] = self._parse_tags(metadata["tags"])
